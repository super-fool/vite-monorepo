  ...
  <div class="view-content-table">
    ...
    >
      ...
      <el-table-column label="结果选择" min-width="250" align="center">
        <template slot-scope="scope">
          <el-select
            v-if="scope.row.strategyType === 1"
            v-model="scope.row.handleType"
            placeholder="请选择"
          >
            <template v-if="scope.row.strategyCode == 4  && scope.row.strategyConfigCode == 5">
              <el-option
                v-for="item in specialOpt"
                size="mini"
                :key="item.code"
                :label="item.type"
                :value="item.code"
          ...
          <el-select
            v-else-if="scope.row.strategyType === 2"
            v-model="scope.row.handleType"
            placeholder="请选择"
          >
            <template v-if="scope.row.strategyCode == 14  && scope.row.strategyConfigCode == 14">
              <el-option
                size="mini"
                v-for="item in specOutResultOptions"
                :key="item.code"
                :label="item.type"
                :value="item.code"
              >
...
export default {
  ...
  created() {},
  mounted() {
    const result = this.$store.getters.dicList.LowVideoStrategyResult;
    this.inResultOptions = result.slice(0, 2);
    this.specialOpt = result.slice(0, 1);
    this.specOutResultOptions = [...result.slice(0, 1), ...result.slice(2)];
  },
  methods: {
    validateForm() {
      let flag = true;
